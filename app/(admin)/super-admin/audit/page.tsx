    ...
    return (
        <div style={{ padding: '2.5rem 3.5rem', width: '100%', maxWidth: '100%' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: '3.5rem' }}>
                <div>
                    <h2 style={{ fontSize: '2rem', fontWeight: '900', color: '#111827', letterSpacing: '-0.04em', margin: 0 }}>Audit Trail (Denetim Kayıtları)</h2>
                    <p style={{ color: '#64748b', marginTop: '8px', fontSize: '1rem', fontWeight: '500' }}>Sistem üzerindeki tüm idari eylemleri, yapılandırma değişikliklerini ve erişim loglarını izleyin.</p>
                </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) 380px', gap: '3rem' }}>
                        ...
                        <div style={{ padding: '28px 40px', borderBottom: '1px solid #f1f5f9', background: '#fff', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
                                <div style={{ background: '#f8fafc', padding: '10px', borderRadius: '12px', border: '1px solid #f1f5f9' }}>
                                    <Database size={20} color="#64748b" strokeWidth={2.5} />
                                </div>
                                <h3 style={{ fontSize: '1.2rem', fontWeight: '900', margin: 0, color: '#111827', letterSpacing: '-0.02em' }}>Olay Akışı</h3>
                            </div>
                            <button className="hover:scale-105 active:scale-95 transition-all" style={{ fontSize: '0.85rem', fontWeight: '900', color: '#ff7a21', background: '#fff7ed', padding: '10px 20px', borderRadius: '12px', border: '1px solid #ffedd5', cursor: 'pointer', letterSpacing: '0.02em' }}>DIŞA AKTAR (.CSV)</button>
                        </div>
                        ...
                        <div style={{ overflowX: 'auto' }}>
                            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                                <thead style={{ background: '#f9fafb' }}>
                                    <tr>
                                        <th style={{ padding: '20px 40px', textAlign: 'left', fontSize: '0.75rem', fontWeight: '900', color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.1em' }}>Zaman Damgası</th>
                                        <th style={{ padding: '20px 40px', textAlign: 'left', fontSize: '0.75rem', fontWeight: '900', color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.1em' }}>Aktör</th>
                                        <th style={{ padding: '20px 40px', textAlign: 'left', fontSize: '0.75rem', fontWeight: '900', color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.1em' }}>Eylem Tipi</th>
                                        <th style={{ padding: '20px 40px', textAlign: 'left', fontSize: '0.75rem', fontWeight: '900', color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.1em' }}>İşlem Detayı</th>
                                        <th style={{ padding: '20px 40px', textAlign: 'right', fontSize: '0.75rem', fontWeight: '900', color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.1em' }}>Kaynak IP</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {loading ? (
                                        ...
                                        <tr>
                                            <td colSpan={5} style={{ padding: '120px', textAlign: 'center' }}>
                                                <div style={{ fontSize: '1.25rem', color: '#94a3b8', fontWeight: '900', letterSpacing: '-0.02em' }}>Henüz bir kayıt bulunmuyor.</div>
                                            </td>
                                        </tr>
                                    ) : logs.map((log) => (
                                        <tr key={log.id} style={{ borderBottom: '1px solid #f8fafc', transition: 'all 0.3s' }} className="group hover:bg-slate-50/50">
                                            <td style={{ padding: '24px 40px' }}>
                                                <div style={{ display: 'flex', alignItems: 'center', gap: '12px', fontSize: '0.95rem', color: '#111827', fontWeight: '900', letterSpacing: '-0.01em' }}>
                                                    <Clock size={16} style={{ color: '#94a3b8' }} strokeWidth={2.5} />
                                            ...
                                            <td style={{ padding: '24px 40px' }}>
                                                <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                                                    <div style={{ width: '32px', height: '32px', borderRadius: '10px', background: '#f1f5f9', color: '#1e293b', fontSize: '0.8rem', display: 'flex', alignItems: 'center', justifyContent: 'center', border: '1px solid #e2e8f0' }}>
                                                        <User size={16} strokeWidth={2.5} />
                                                    </div>
                                                    <span style={{ fontSize: '0.95rem', fontWeight: '900', color: '#111827' }}>{log.adminEmail || 'Sistem'}</span>
                                                </div>
                                            </td>
                                            <td style={{ padding: '24px 40px' }}>
                                                <span style={{
                                            ...
                                            <td style={{ padding: '24px 40px' }}>
                                                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', maxWidth: '350px' }}>
                                                    <Info size={16} style={{ flexShrink: 0, color: '#94a3b8' }} strokeWidth={2} />
                                                    <span style={{ fontSize: '0.9rem', color: '#475569', fontWeight: '700', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', letterSpacing: '-0.01em' }}>{log.details}</span>
                                                </div>
                                            </td>
                                            <td style={{ padding: '24px 40px', textAlign: 'right' }}>
                                                <span style={{ fontFamily: 'monospace', fontSize: '0.85rem', color: '#94a3b8', fontWeight: '900', background: '#f8fafc', padding: '4px 10px', borderRadius: '8px', border: '1px solid #f1f5f9' }}>{log.ipAddress || '-'}</span>
                                            </td>
                                        </tr>
                                    ))}
                                </tbody>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '2.5rem', position: 'sticky', top: '2.5rem', height: 'fit-content' }}>
                    {/* Log Integrity Card */}
                    <div className="card" style={{ border: 'none', padding: '36px', background: 'linear-gradient(135deg, #0f172a 0%, #1e293b 100%)', color: '#fff', borderRadius: '32px', boxShadow: '0 20px 30px -10px rgba(0, 0, 0, 0.3)', position: 'relative', overflow: 'hidden' }}>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '14px', marginBottom: '24px', position: 'relative', zIndex: 1 }}>
                            ...
                            <h3 style={{ margin: 0, fontSize: '0.95rem', fontWeight: '900', textTransform: 'uppercase', letterSpacing: '0.15em' }}>Kayıt Bütünlüğü</h3>
                        </div>
                        <p style={{ margin: 0, fontSize: '0.95rem', color: '#94a3b8', lineHeight: '1.7', fontWeight: '500', position: 'relative', zIndex: 1 }}>
                    ...
                    <div className="card" style={{ border: 'none', padding: '32px', background: '#fff', borderRadius: '32px', boxShadow: '0 10px 15px -3px rgba(0, 0, 0, 0.05)' }}>
                        <h4 style={{ margin: '0 0 24px 0', fontSize: '1rem', fontWeight: '900', color: '#111827', textTransform: 'uppercase', letterSpacing: '0.1em' }}>Olay Analizi</h4>
